          ...
          if ((rankNum > 0) && (rankNum < 201)) {
            ...
            while(maleFemRankLoop == 0){
              ...
              if((atoi(userResponse)) == 0){
                printf("Rank %d: %d Male:%s (%d) \n %d: Male: %s %d",rankNum,decade1, popular.maleName[rankNum-1], popular.maleNumber[rankNum-1],decade2,popularYear2.maleName[rankNum-1],popularYear2.maleNumber[rankNum-1]);
                maleFemRankLoop = 1;
              }

              else if((atoi(userResponse)) == 1){
                printf("Rank %d: %d Female:%s (%d) \n %d: Female: %s %d",rankNum, decade1, popular.femaleName[rankNum-1], popular.femaleNumber[rankNum-1]), decade2,popularYear2.femaleName[rankNum-1],femaleNumber.popularYear2[rankNum-1];
                maleFemRankLoop = 1;
              }
